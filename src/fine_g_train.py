...
import os, re, yaml, pickle, json, jsonlines, logging, time, math
...
#configs
config_file = f'{main_path}/src/configs/second_config.yaml'
...
training_config = configs['fine_training']
...
tokenizer_path = os.path.join(configs['artifact_folder'], 'vocab_remi_sikim.pkl')
with open(tokenizer_path, 'rb') as f:
    tokenizer = pickle.load(f)
vocab_size = len(tokenizer)

caption_path = configs['raw_data']['gukak']['caption_path']
...
with accelerator.main_process_first():
    dataset_path = configs['raw_data']['gukak']['folder_path']
...
# model load
pre_output_dir = os.path.join(os.path.dirname(output_dir), 'pre_g_output')
epoch_dirs = [d for d in os.listdir(pre_output_dir) if re.match(r'epoch_(\d+)', d)]
latest_epoch = max(epoch_dirs, key=lambda d: int(re.search(r'\d+', d).group()))
model_path = os.path.join(pre_output_dir, latest_epoch, 'kot2m_model.bin')
